...
const domaincf = "premm.my.id"; 
const iniemail = "ajanggaul9@gmail.com";

// Daptar admin anu diidinan ngakses bot
const adminIds = [5881666389, 987654321]; // Ganti jeung user_id admin anu diidinan
...
bot.on('text', (ctx) => {
    ...
    if (status === 'nungguan_ip') {
        ...
        userContext[userId] = { status: 'nungguan_domain', ip: ip };
        ctx.reply('⚠️ *Pilih cara pikeun domain:*\n1. Manual\n2. Random', {
            parse_mode: 'Markdown',
            ...Markup.inlineKeyboard([
                [Markup.button.callback('Manual', 'domain_manual')],
                [Markup.button.callback('Random', 'domain_random')]
            ]).resize()
        });
    }
});

bot.action('domain_manual', (ctx) => {
    if (!isAdmin(ctx.from.id)) {
        return ctx.reply('⚠️ Anjeun teu gaduh idin pikeun ngagunakeun bot ieu.');
    }
    const userId = ctx.from.id;
    userContext[userId].status = 'nungguan_domain_manual';
    ctx.reply('⚠️ *Mangga kirimkeun domain anu rék didaptarkeun.*', { parse_mode: 'Markdown' });
});

bot.action('domain_random', (ctx) => {
    if (!isAdmin(ctx.from.id)) {
        return ctx.reply('⚠️ Anjeun teu gaduh idin pikeun ngagunakeun bot ieu.');
    }
    const userId = ctx.from.id;
    ...
    if (userState && userState.status === 'nungguan_domain') {
        const ip = userState.ip;
        const domain = `${Math.random().toString(36).substring(2, 7)}.${domaincf}`;
        userContext[userId] = { status: 'nungguan_proxied', ip: ip, domain: domain };
...
bot.on('text', (ctx) => {
    ...
    if (status === 'nungguan_domain_manual') {
        const domain = ctx.message.text;
        userContext[userId] = { status: 'nungguan_proxied', ip: userContext[userId].ip, domain: domain };
        ctx.reply('⚠️ *Naha anjeun hoyong ngaktipkeun proxy keur DNS record ieu?*', {
            parse_mode: 'Markdown',
            ...Markup.inlineKeyboard([
                [Markup.button.callback('Enya', 'proxied_true')],
